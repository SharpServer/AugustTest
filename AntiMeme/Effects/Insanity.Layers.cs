...
public partial class Insanity
{
    ...
    private sealed class RoamerField
    {
        ...
        public RoamerField(PlayerDisplay display, Player player, Random rng)
        {
            _rng = rng;
    ...
    private sealed class MessageContext
    {
        ...
        public string Resolve(string body, Random rng)
        {
            ...
            if (Contains(body, TimeToken))
                body = body.Replace(TimeToken, Round.Duration.ToString(@"mm\:ss"));
        ...
        private string ResolveOtherNickname(Random rng)
        {
            ...
            {
                // NPC / ダミーの名前を出すと「居ない誰か」を名指ししてしまうので実プレイヤーだけ。
                if (!other.IsPlayer || other == _player)
                    continue;
        ...
        /// <summary>
        /// {ZONE} 用の日本語名。トークンを読ませるためだけの表なので Insanity の中に閉じておく。
        /// </summary>
        private static string TranslateZone(FacilityZone zone) => zone switch
        {
            FacilityZone.Surface => "地上",
            FacilityZone.Entrance => "エントランス",
            FacilityZone.HeavyContainment => "重度収容区画",
            FacilityZone.LightContainment => "軽度収容区画",
            FacilityZone.Other => "収容施設外",
            _ => "■■■",
        };

        /// <summary>{ROOM} 用の日本語名。</summary>
        private static string TranslateRoom(Room room) => RoomNames.Of(room, "■■■");
    }
}
