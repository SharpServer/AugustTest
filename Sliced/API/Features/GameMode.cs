...
/// <summary>
/// そのラウンド 1 回だけ走る特殊ルールです(旧 SpecialEvent)。
///
/// 遅延処理の打ち切り判定は基底が持ちます。
/// コルーチンの中で <see cref="IsCanceled"/> を見るか、
/// <see cref="Scope"/> に載せておけばラウンド終了で勝手に止まります。
/// 旧実装のように各イベントが世代カウンタを自前で持つ必要はありません。
/// </summary>
/// <example>
/// <code>
...
///
///         Scope.Delay(120f, () => Announce("撤退せよ"));
///     }
/// }
/// </code>
/// </example>
public abstract class GameMode
{
    ...
    /// 季節限定などの条件をここに書きます。
    /// </summary>
    public virtual bool IsAvailable => Player.ReadyList.Count() >= MinimumPlayers;
    ...
    public static IReadOnlyList<GameMode> All()
    {
        ...
        foreach (Type type in TypeParser.FindTypes<GameMode>())
        {
            ...
            {
                Logger.Error($"[Sliced] ゲームモード {type.FullName} を生成できませんでした: {exception}");
            }
        }
    ...
    public static void StopCurrent()
    {
        ...
        {
            Logger.Error($"[Sliced] {mode.GetType().Name} の OnStopped で例外が発生しました: {exception}");
        }
    }
    ...
    public static void Shutdown()
    {
        ...
        ServerEvents.RoundRestarted -= StopCurrent;
        hooked = false;
    }
    ...
    public bool Start()
    {
        ...
        {
            Logger.Error($"[Sliced] {GetType().Name} の起動に失敗しました: {exception}");
            StopCurrent();
