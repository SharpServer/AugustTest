...
///
/// 使うのは「LabApi や EXILED を通さず自分で <see cref="NetworkConnection"/> に書き込む」
/// 場面だけです。普通のプレイヤー操作には要りません。
/// そちらは <c>player is { IsDestroyed: false }</c> で足ります。
///
/// LabApi の <see cref="Player.IsReady"/> は
/// <c>InstanceMode != Unverified &amp;&amp; nicknameSync.NickSet</c> であって
/// <see cref="ClientInstanceMode.ReadyClient"/> を見ていないため、
/// 生の送信経路ではこちらを使います。
/// </summary>
public static class NetGuards
{
    ...
    ///
    /// <see cref="NetworkConnection.isReady"/> だけでは不十分で、
    /// <see cref="ClientInstanceMode.ReadyClient"/> であることも必要です。
    ...
    public static bool IsReadyClient(Player player) =>
        player is { IsDestroyed: false } && IsReadyClient(player.ReferenceHub);
