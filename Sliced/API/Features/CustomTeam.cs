...
///
/// enum も string キーもレジストリも要りません。型そのものが同一性です。
...
public abstract class CustomTeam
{
    ...
    /// 放送や勝敗文で使う <see cref="Name"/> と別の綴りを出したい陣営だけ override します。
    /// </summary>
    ...
    public IEnumerable<Player> Members =>
        Player.ReadyList.Where(player => player.IsAlive && Includes(player));
    ...
    private static CustomTeam[] Build()
    {
        foreach (Type type in TypeParser.FindTypes<CustomTeam>())
        {
            ...
            {
                Logger.Error($"[Sliced] チーム {type.FullName} を生成できませんでした: {exception}");
            }
        }
