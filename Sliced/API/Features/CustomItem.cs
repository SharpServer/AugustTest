...
///
/// 識別子の文字列も専用 enum もありません。型そのものが同一性です。
/// </summary>
/// <example>
/// <code>
...
public abstract class CustomItem
{
    ...
    /// </summary>
    public Player Owner => Item?.CurrentOwner;
    ...
    {
        if (custom is null || player is not { IsDestroyed: false }) return null;
    ...
    private static CustomItem Instantiate(Type type)
    {
        ...
        {
            Logger.Error($"[Sliced] {type?.FullName ?? "null"} はカスタムアイテムとして生成できません。");
        ...
        {
            Logger.Error($"[Sliced] カスタムアイテム {type.FullName} の生成に失敗しました: {exception}");
    ...
    private void AttachPickupModel()
    {
        ...
        {
            Logger.Warn($"[Sliced] {GetType().Name} のスキマティック '{model}' を出せませんでした。");
    ...
    private void ShowItemHint(Player player, string text, float duration)
    {
        if (player is not { IsDestroyed: false } || string.IsNullOrEmpty(text)) return;
    ...
    private void Invoke(Action action, string name)
    {
        ...
        {
            Logger.Error($"[Sliced] {GetType().Name}.{name} で例外が発生しました: {exception}");
        }
    }
    ...
    private static void OnAnyPickedUp(PlayerPickedUpItemEventArgs ev)
    {
        if (Of(ev.Item) is { } custom)
        {
            ...
            custom.DetachPickupModel();
            custom.Customize(ev.Item);
            custom.Invoke(() => custom.OnPickedUp(ev.Player), nameof(OnPickedUp));
            custom.ShowItemHint(ev.Player, custom.PickupHint, custom.PickupHintDuration);
        }
    }
    ...
    private static void OnAnyDropped(PlayerDroppedItemEventArgs ev)
    {
        if (Of(ev.Pickup) is { } custom)
        {
            ...
            custom.Invoke(() => custom.OnDropped(ev.Player), nameof(OnDropped));
            custom.AttachPickupLight();
            custom.AttachPickupModel();
        }
    }
    ...
    private static void OnAnyChangedItem(PlayerChangedItemEventArgs ev)
    {
        if (Of(ev.OldItem) is { } unequipped)
            unequipped.Invoke(() => unequipped.OnUnequipped(ev.Player), nameof(OnUnequipped));

        if (Of(ev.NewItem) is { } equipped)
        {
            equipped.Invoke(() => equipped.OnEquipped(ev.Player), nameof(OnEquipped));
