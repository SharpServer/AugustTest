...
public abstract class SpawnSet
{
    ...
    ///
    /// ラウンド開始時 (LabApi の RoundStarted) はバニラの役職割り当て
    /// (PlayerRoles.RoleAssign.RoleAssigner) より前に発火するため、
    /// その時点のプレイヤーは Spectator ではなく RoleTypeId.None (ロビー) である。
    /// ラウンド中に呼ばれた場合は Spectator が対象になる。
    /// Overwatch は RoleTypeId.Overwatch なのでここには含まれない。
    /// </summary>
    ...
    private int SpawnInternal()
    {
        ...
        {
            Logger.Error($"SpawnSet '{Name}' の AllowedPlayerCount が不正です: {AllowedPlayerCount} (-1 以上にしてください)");
    ...
    private bool TrySpawn(SpawnRoleState state, Player target)
    {
        ...
        {
            Logger.Error($"SpawnSet '{Name}' の役職割り当てで例外が発生しました: {exception}");
