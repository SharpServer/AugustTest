...
///
/// 寿命は <see cref="PlayerScope"/> に相乗りします。プレイヤーの退出・ラウンド再開・
/// 役職変更で自動的に失われます。
/// </summary>
/// <example>
/// <code>
...
{
    private static readonly Dictionary<Player, List<AbilityBase>> Granted = new Dictionary<Player, List<AbilityBase>>();
    ...
    {
        if (player is not { IsDestroyed: false }) return null;
        ...
        if (!Granted.TryGetValue(player, out List<AbilityBase> abilities))
        {
            abilities = [];
            Granted[player] = abilities;

            // 寿命は PlayerScope に任せる。独自のイベントフックは増やさない。
            PlayerScope.Of(player).OnDispose(owner => Granted.Remove(owner));
        }
        ...
        {
            Logger.Error($"[Sliced] {ability.GetType().Name} の OnGranted で例外が発生しました: {exception}");
        }
    ...
    {
        if (player is not null && Granted.TryGetValue(player, out List<AbilityBase> abilities))
            return abilities;
    ...
    public static void Revoke<T>(Player player) where T : AbilityBase
    {
        ...
        if (!Granted.TryGetValue(player, out List<AbilityBase> abilities)) return;
    ...
    {
        if (player is null || !Granted.TryGetValue(player, out List<AbilityBase> abilities)) return;
    ...
    public bool TryUse(out string failureReason)
    {
        ...
        {
            Logger.Error($"[Sliced] {GetType().Name} の OnUsed で例外が発生しました: {exception}");
        }
    ...
    {
        if (Player is not { IsDestroyed: false } || !Player.IsAlive)
        {
