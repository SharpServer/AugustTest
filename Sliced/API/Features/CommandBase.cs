...
public abstract class CommandBase : ICommand
{
    ...
    /// 旧実装の RootCommand が 30 行の RegisterCommand を抱えていたのがこれで消えます。
    /// </summary>
    ...
    /// LabAPI の権限 (<c>PermissionsManager</c>) やアセンブリ名を接頭辞にしたノードへ替えると、
    ...
    ///
    /// ID → UserId → 表示名の完全一致 → 表示名の部分一致 の順で探します。
    /// </summary>
    protected bool TryGetPlayer(int index, out Player player)
    {
        ...
        player ??= Player.GetByDisplayName(raw, requireFullMatch: true);
        player ??= Player.GetByDisplayName(raw);
